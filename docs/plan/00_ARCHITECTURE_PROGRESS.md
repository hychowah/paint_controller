...
- Most recent completed slice: Workstream E1 shell/connectivity panel contract reduction
- Last completed implementation slice: Workstream E1 shell/connectivity panel contract reduction
...
- Current active retirement family: shell/connectivity telemetry is now active in progress after the shared shell `ConnectionStatusPanel.qml` seam moved behind bounded `shellConnectivityStatus`, while the shared video subtree remains partially retired behind `videoRuntime`, the shared winch seam is behind `winchStatus`, the shared teensy seams are behind `teensyStatus`, the shared wheel summary/control seam is behind `wheelStatus`, and the shared recording seam is behind `recordingStatus`
- Next retirement family after that: explicit shell/home/launcher follow-up within the same shell/connectivity family, then opportunistic settings cleanup or bounded device/status remainder only if the touched slice proves higher leverage
...
| Workstream E — Contract-first infrastructure and downstream automation | active in progress | E1 slices 1-2 retired the workflow/editor and system-control command globals behind `systemControlServices`; the shared video slice partially retired the touched fullscreen-video root behind `videoRuntime`; the first device/status sub-slice moved the shared winch summary and power/load seam behind `winchStatus`; the second moved the shared teensy power/header seam behind `teensyStatus`; the third moved the shared wheel summary/control seam behind `wheelStatus`; the fourth moved the shared recording seam behind `recordingStatus`; the fifth moved the shared system-control teensy feature-toggle seam behind bounded `teensyStatus` fields; and the sixth started the shell/connectivity family by moving the shared `ConnectionStatusPanel.qml` shell chrome behind bounded `shellConnectivityStatus` | Continue shell/connectivity with the bounded `PageHome.qml` status-header follow-up while keeping launcher commands/config and video preview explicit remainder, then revisit explicit device/status remainder or opportunistic settings cleanup only if the touched slice proves higher leverage | Keep `AppRuntime` as the composition root, but any touched slice must retire at least one app-scope QML read path, must publish a before/after retirement ledger, must not widen `ShellState`, `OverlayHostPolicy`, or the global context bag, and must not count namespacing alone as convergence |
...
- Feature roots such as `SystemControlWorkspace.qml` are materially cleaner than before, the shared video subtree now consumes a bounded `videoRuntime` contract for the touched control, frame-refresh, and top-bar seams, the first five device/status sub-slices consume `winchStatus`, bounded `teensyStatus` seams, `wheelStatus`, plus `recordingStatus`, and the shared shell panel now consumes `shellConnectivityStatus`; the remaining shell/home/launcher remainder and explicit device/status remainder are the next primary E1 targets.
...
1. Continue the shell/connectivity telemetry family with the bounded `PageHome.qml` status-header slice: retire the next shell-facing availability/heartbeat read seam without widening `shellConnectivityStatus` into a generic shell bag or pulling `PageLauncher.qml` commands/config editing plus video preview into the same checkpoint.
...
5. Keep `PageLauncher.qml` command/config editing, `PageHome.qml` video preview, remaining fullscreen telemetry, and untouched device/status callers explicit remainder unless a touched slice deliberately picks them up; settings cleanup should stay opportunistic behind the active shell/connectivity family.
