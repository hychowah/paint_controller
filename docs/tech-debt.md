...
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current family status. Treat completed stages through Stage 4.5 plus completed Workstreams A through D as historical record. Under Workstream E, the shared video/runtime family is already retired for the touched subtree, the first two device/status sub-slices are already retired behind `winchStatus` and `teensyStatus`, and the current active family remains device/status in further bounded sub-slices. Settings cleanup stays next only where it still materially reduces ambient reads, and narrower automation-contract work stays downstream of those contract-reduction checkpoints.
