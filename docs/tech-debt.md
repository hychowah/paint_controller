...
**Priority**: medium
**Effort**: high
**Why it matters**: The next major debt is no longer best described as a single "deep QML structural rebuild". The validated roadmap now prioritizes architecture boundaries in verified risk order, and the core maintainability gap has shifted from unsafe QML writes toward making the remaining operator-facing contracts smaller and more explicit. Stage 1, Stage 2, Stage 3A, Stage 3B1, Stage 4, Stage 4.5, Workstream A, Workstream B, Workstream C, and Workstream D are now complete for the targeted families: the settings family consumes typed owner helpers instead of raw property-bag semantics, and unused app-scope QML context exposure is reduced. The remaining debt is future automation seam work plus downstream feature-shell/design cleanup.
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active workstream roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current checkpoint status. Treat completed stages through Stage 4.5 plus completed Workstreams A through D as historical record. Proceed with Workstream E future automation seam design next, then the downstream product/design streams.
