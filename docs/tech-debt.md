...
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current family status. Treat completed stages through Stage 4.5 plus completed Workstreams A through D as historical record. Under Workstream E, the shared video/runtime family is only partially retired for the touched subtree and any remaining fullscreen telemetry callers must stay explicitly quarantined until a later dedicated follow-up. The first five device/status sub-slices are already retired behind `winchStatus`, bounded `teensyStatus` seams, `wheelStatus`, and `recordingStatus`, and the tracked shell/connectivity family is now completed for the shared shell panel plus touched PageHome and Launcher seams behind `shellConnectivityStatus` plus `launcherAdmin`, with raw `sshHandler` removed from the root QML context contract. The next bounded follow-up is to resume explicit domain telemetry remainder by wheel, winch, teensy, then lidar/monitor unless a touched slice proves a different order is higher leverage, while PageHome video preview stays explicit remainder. Settings cleanup stays after that unless a touched slice proves higher leverage, and narrower automation-contract work stays downstream of those proof-backed contract-reduction checkpoints.
