...
**Priority**: medium
**Effort**: high
**Why it matters**: The next major debt is no longer best described as a single "deep QML structural rebuild". The validated roadmap now prioritizes architecture boundaries in verified risk order, with Stage 1 and Stage 2 closed: command, device, workflow, and overlay/input ownership boundaries are frozen; the remaining Settings quick-apply path is not; shell policy and route hardening are split around a Settings-truthfulness pass; and workflow work is split between present-system stabilization and future behavior-tree preparation.
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active staged roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current status. Treat Stage 1 command, device, and workflow work as complete, treat Stage 2 overlay/input ownership freeze as complete, and treat Stage 3A shell policy as complete. Proceed through Stage 4A Settings truthfulness next, then Stage 3B route hardening, Stage 4B broader settings authority, Stage 5 overlay contract, and Stage 6A/6B workflow work.
