...
**Priority**: medium
**Effort**: high
**Why it matters**: The next major debt is no longer best described as a single "deep QML structural rebuild". The validated roadmap now prioritizes architecture boundaries in verified risk order, and the core maintainability gap has shifted from unsafe QML writes toward making the remaining operator-facing contracts smaller and more explicit. Stage 1, Stage 2, Stage 3A, Stage 3B1, Stage 4, Stage 4.5, Workstream A, Workstream B, and Workstream C are complete for the touched families: command/device/workflow boundaries are frozen, overlay/input ownership is frozen, shell policy is explicit, top-level route identity is now key-first and canonical in the shell, the Settings route is truthful where schema-backed settings exist, the remaining tracked direct-admin QML actuation paths are behind Python-owned boundaries, the workflow runtime/editor contract is stabilized, and the touched overlay host plus legality seams are explicit. The remaining debt is continued contract retirement plus later design/feature-shell cleanup.
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active workstream roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current checkpoint status. Treat completed stages through Stage 4.5 plus completed Workstreams A, B, and C as historical record. Proceed with Workstream D dedicated QML-contract reduction next, then the later contract/design streams.
