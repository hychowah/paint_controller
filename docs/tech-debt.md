...
**What to do**: Treat `docs/plan/00_ARCHITECTURE_PROGRESS.md` as the only live execution board and `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as durable architecture guidance. The next execution order is family-based: finish the remaining `PageHome.qml` preview/frame-refresh cleanup inside the video/runtime review, then revisit settings cleanup only if it still produces a real surface-area reduction. Keep the shell and launcher families frozen, keep the explicit quarantine list visible by file, keep `PageHome.qml` preview as remainder until that family is active, and do not open the automation follow-on until the app-scope QML contract is materially smaller.
