...
**Priority**: medium
**Effort**: high
**Why it matters**: The next major debt is no longer best described as a single "deep QML structural rebuild". The validated roadmap now prioritizes architecture boundaries in verified risk order: dangerous QML-to-application action paths, the overlay/control cycle, the narrower two-surface shell split, settings authority and capability classes, then overlay formalization, workflow stabilization, and only then narrower QML contracts.
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active staged roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current status. Stage 1A through Stage 1E are complete, and Stage 2 is now materially underway: the live `OverlayController` / `ControlProcessor` cycle is removed and joystick selection ownership now lives in `JoystickSelectionModel`. The remaining Stage 2 work is to narrow `OverlayController` further toward presentation/compatibility duties without changing operator behavior.
