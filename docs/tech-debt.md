...
**Priority**: medium
**Effort**: high
**Why it matters**: The next major debt is no longer best described as a single "deep QML structural rebuild". The validated roadmap now prioritizes architecture boundaries in verified risk order, and the core maintainability gap has shifted from unsafe QML writes toward making the remaining operator-facing contracts smaller and more explicit. Stage 1, Stage 2, Stage 3A, Stage 3B1, Stage 4, Stage 4.5, and Workstream A are complete: command/device/workflow boundaries are frozen, overlay/input ownership is frozen, shell policy is explicit, route ownership is now canonical in the shell, the Settings route is truthful where schema-backed settings exist, the remaining tracked direct-admin QML actuation paths are behind Python-owned boundaries, and the workflow runtime/editor contract is stabilized. The remaining debt is overlay host/legality formalization, later route formalization, and continued contract retirement.
**What to do**: Follow `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md` as the active workstream roadmap and `docs/plan/00_ARCHITECTURE_PROGRESS.md` for current checkpoint status. Treat completed stages through Stage 4.5 and completed Workstream A as historical record. Proceed with Workstream B1 overlay hosting/layering next, then Workstream B2 operator-action legality, Workstream C route formalization, and the later contract/design streams.
