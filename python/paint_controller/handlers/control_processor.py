...
class ControlProcessor(QObject):
        ...
        if self._can_send_message():
            ...
            self.left_control_value = (
                ...
                if isinstance(left_value, str)
                    ...
                    f"Fx:{left_value[0]:.1f} Fy:{left_value[1]:.1f}"
                    if isinstance(left_value, tuple)
                    else (f"L:{left_value:.1f}" if isinstance(left_value, (int, float)) else str(left_value))
                )
            )
            ...
            self.right_control_value = (
                ...
                if isinstance(right_value, str)
                    ...
                    f"Fx:{right_value[0]:.1f} Fy:{right_value[1]:.1f}"
                    if isinstance(right_value, tuple)
                    else (f"R:{right_value:.1f}" if isinstance(right_value, (int, float)) else str(right_value))
                )
            )
    ...
    def _process_wheel_travel(self, input_state: dict[str, Any], mode: str, stick: str) -> None:
        ...
        self._left_wheel_travel_mm, self._right_wheel_travel_mm = wheel_travel_teleop.accumulate_wheel_travel(
            input_state,
            mode,
            stick,
