...
class PointCloudSubscriber(Node, QObject):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(PointCloud2, "/unilidar/cloud", self.pointcloud_callback, 10)
    ...
    def pointcloud_callback(self, msg):
        ...
        for i in range(0, num_points, downsample):
            ...
            if not (np.isnan(x) or np.isnan(y) or np.isnan(z) or abs(x) > 100 or abs(y) > 100 or abs(z) > 100):
