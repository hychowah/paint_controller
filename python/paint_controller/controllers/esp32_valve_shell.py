#!/usr/bin/env python3
"""ESP32 valve Qt transport shell (Level C P5).

Pure protocol/HAL lives in ``esp32_valve`` (no PySide6). This module owns
UDPReceiveThread, timers, discovery, and presentation Signals.
"""
...
from paint_controller.controllers.esp32_valve import (
    ...
    START_BYTE as PROTOCOL_START_BYTE,
)
...
class ESP32ValveController(QObject):
    ...
    def __init__(self, node: Node) -> None:
        ...
        self._hal = Esp32ValveHal(notifier=self._notifier)
    ...
    def _command_callback(self, msg: Float32):
        ...
        self.setValveTurn(float(msg.data))
    ...
    def _handle_status(self, status: dict):
        """Handle received status from UDP thread (main/Qt thread)."""
        self._hal.apply_status(status)

    def _handle_connection_lost(self):
        """Handle connection timeout"""
        was = self._hal.esp32_connected
        self._hal.mark_connection_lost()
        if was:
            logging.debug("ESP32 connection lost")

    def _check_reconnect(self):
        """Periodically check and attempt reconnection"""
        if not self._hal.esp32_connected and not self._discovery_inflight:
    ...
    def _send_keepalive(self):
        ...
        if not self._hal.esp32_connected:
            return
        position_raw = self._hal.keepalive_raw_position()
    ...
    def _publish_status(self):
        ...
        msg = ValveStatus()
        msg.valve_motor_current = self._hal.valve_motor_current
        msg.valve_position = self._hal.valve_position
        msg.valve_rate = self._hal.valve_rate
        msg.total_volume = self._hal.total_volume
        msg.valve_motor_connected = self._hal.valve_motor_connected
        msg.flow_meter_connected = self._hal.flow_meter_connected
    ...
    def _build_message(self, msg_type: MessageType, payload: bytes) -> bytes:
        ...
        from paint_controller.controllers.esp32_valve import build_udp_message

        return build_udp_message(msg_type, payload, start_byte=self.START_BYTE)
    ...
    def _send_turn_valve(self, position: int):
        ...
        if not self._esp32_ip:
            return
        try:
            message = build_turn_valve_message(position, start_byte=self.START_BYTE)
            with self._sock_lock:
                if self._sock:
    ...
    def setValveTurn(self, position_pct: float):
        """Set valve position (percent). Delegates to pure HAL + transport send."""
        position_raw = self._hal.setValveTurn(position_pct)
        self._send_turn_valve(position_raw)
    ...
    # Test/compat aliases for HAL-owned command/connection tracking
    @property
    def _last_command(self) -> float:
        return self._hal._last_command

    @_last_command.setter
    def _last_command(self, value: float) -> None:
        self._hal._last_command = float(value)

    @property
    def _last_command_time(self) -> float:
        return self._hal._last_command_time

    @_last_command_time.setter
    def _last_command_time(self, value: float) -> None:
        self._hal._last_command_time = float(value)

    @property
    def _esp32_connected(self) -> bool:
        return self._hal.esp32_connected

    @_esp32_connected.setter
    def _esp32_connected(self, value: bool) -> None:
        self._hal._esp32_connected = bool(value)

    # Plain Python properties (Level B) — QML surface is ValveStatus.
    @property
    def valve_position(self) -> float:
        return self._hal.valve_position

    @property
    def valve_rate(self) -> float:
        return self._hal.valve_rate

    @property
    def total_volume(self) -> float:
        return self._hal.total_volume

    @property
    def valve_motor_current(self) -> int:
        return self._hal.valve_motor_current

    @property
    def valve_motor_connected(self) -> bool:
        return self._hal.valve_motor_connected

    @property
    def flow_meter_connected(self) -> bool:
        return self._hal.flow_meter_connected
