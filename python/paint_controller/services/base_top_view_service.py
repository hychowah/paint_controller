...
class BaseTopViewTransformer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if os.path.exists(config_path):
            try:
                ...
                logging.info(f"Loaded base top view config: k1={self.k1}, k2={self.k2}, k3={self.k3}, k4={self.k4}, zoom={self.zoom}")
        ...
        self.src_points_normalized: List[List[float]] = [
            [0.012, 1.0], [0.988, 1.0], [0.837, 0.727], [0.372, 0.727]
        ]
    ...
    def initialize_for_resolution(self, width: int, height: int):
        ...
        logging.info(f"Base top view output: {side}x{side} square "
    ...
    def _compute_remap_tables(self):
        ...
        logging.info(f"Scaled calibration: center=({scaled_center_x:.0f},{scaled_center_y:.0f}) "
        ...
        self.camera_matrix = np.array([
            [focal_for_fisheye, 0, scaled_center_x],
            [0, focal_for_fisheye, scaled_center_y],
            [0, 0, 1]
        ], dtype=np.float32)
...
class BaseTopViewWorker(QObject):
    ...
    def process_frame(self, camera_type: CameraType, qimage: QImage):
        ...
        try:
            ...
            if not self._initialized or width != self.transformer._input_width or height != self.transformer._input_height:
...
class BaseTopViewService(QObject):
    ...
    def __init__(self, video_handler: VideoStreamHandler, settings_manager=None, parent: Optional[QObject] = None):
        ...
        if self.settings_manager:
            ...
            src_points = self.settings_manager.get("base_top_view_src_points", [[0.012, 1.0], [0.988, 1.0], [0.837, 0.727], [0.372, 0.727]])
