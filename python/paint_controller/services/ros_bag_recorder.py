...
class RosBagRecorder(QObject):
    ...
    def __init__(self, show_popup_fn=None, parent: QObject | None = None):
        ...
        self._current_bag_folder: str | None = None

        # SSH configuration
        self._ssh_launcher: SSHLauncher | None = None
    ...
    def _run_ssh_command(self, command: str, callback: Callable[[str, str], None] | None = None) -> None:
        ...
        def on_start_complete(stdout: str, stderr: str):
            if stderr and "error" in stderr.lower():
                ...
                if self._show_popup_fn:
                    self._show_popup_fn(
                        "ROS Bag Recording", f"Failed to start recording: {stderr[:100]}", "error", 3000
                    )
                return
            ...
            # Show success popup
            if self._show_popup_fn:
                self._show_popup_fn("ROS Bag Recording", f"Recording started: {self._current_bag_folder}", "info", 2000)
    ...
    def _compress_bag_folder(self) -> None:
        ...
        def on_compress_complete(stdout: str, stderr: str):
            ...
            if stderr and "error" in stderr.lower():
                ...
                if self._show_popup_fn:
                    self._show_popup_fn("ROS Bag Recording", f"Compression failed: {stderr[:100]}", "error", 3000)
            else:
                ...
                if self._show_popup_fn:
                    self._show_popup_fn("ROS Bag Recording", f"Saved: {folder_name}.tar.gz", "info", 2000)
    ...
    def toggleBagRecording(self) -> None:
        ...
        if self._is_compressing:
            ...
            if self._show_popup_fn:
                self._show_popup_fn("ROS Bag Recording", "Please wait for compression to complete", "warning", 2000)
            return
    ...
    def startBagRecording(self) -> None:
        ...
        if not self._ssh_launcher:
            ...
            if self._show_popup_fn:
                self._show_popup_fn(
                    "ROS Bag Recording", "SSH connection not configured for End Effector", "error", 3000
                )
            return
    ...
    def cleanup(self) -> None:
        ...
        if self._is_bag_recording:
            ...
            if self._remote_pid:
                stop_command = f"kill -2 {self._remote_pid} 2>/dev/null || pkill -2 -f 'ros2 bag record' 2>/dev/null"
            else:
