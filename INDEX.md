...
- The current architecture direction is tracked in **`docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md`** and the current slice status is tracked in **`docs/plan/00_ARCHITECTURE_PROGRESS.md`**. **Stage 1A through Stage 1E, Stage 2, and Stage 3A are complete**: the live `OverlayController` / `ControlProcessor` cycle is gone, joystick selection ownership lives in `JoystickSelectionModel`, shell policy now lives in a narrow `ShellState`, and the next recommended implementation path is **Stage 4A settings truthfulness** before Stage 3B route hardening.
...
- Latest verified local validation on 2026-04-26 is green at `205 passed` for `python/paint_controller/venv/bin/python -m pytest tests -q`. Focused Stage 3A shell-policy slices are green for `tests/test_shell_state.py`, `tests/test_startup_smoke.py`, `tests/test_services_runtime.py`, and `tests/test_qt_bridge.py`; focused Stage 2 ownership and regression slices also remain green.
...
- **Test suite**: latest verified local full-suite status is `205 passed` on 2026-04-26; focused Stage 3A shell-policy slices and Stage 2 ownership/regression slices are also green; revalidate with the venv python before commit if you need a fresher claim
