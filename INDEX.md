...
- Latest verified local validation on 2026-04-27 is green at `252 passed` for `python/paint_controller/venv/bin/python -m pytest tests -q`. Recent focused validation is also green at `26 passed` for `tests/test_startup_smoke.py`, `tests/test_qml_imports.py`, and `tests/test_workflow_runner.py`, and at `4 passed` for `tests/test_ssh.py`.
...
- **Test suite**: latest verified local full-suite status is `252 passed` on 2026-04-27; recent focused fullscreen overlay hardening is green at `26 passed`, and the SSH teardown regression slice is green at `4 passed`; revalidate with the venv python before commit if you need a fresher claim
