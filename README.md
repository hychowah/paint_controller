...
- The current architecture roadmap is in `docs/plan/01_PYTHON_QT_ARCHITECTURE_DEBT_PLAN.md`. Stage 1A through Stage 1E, Stage 2, Stage 3A, Stage 3B1, Stage 4, Stage 4.5, Workstream A, Workstream B, and Workstream C are complete for the touched families: command/device/workflow boundaries are frozen, shell and route ownership are explicit, top-level route identity is now key-first in the shell, the Settings route is truthful where schema-backed settings exist, the remaining tracked direct-admin QML mutators are behind Python-owned boundaries, the workflow runtime/editor contract is stabilized behind Python-owned read/write seams, and the touched overlay host plus operator-legality seams are now explicit.
...
- Latest verified local validation on 2026-04-26 is green at `245 passed` via `python/paint_controller/venv/bin/python -m pytest tests -q`.
