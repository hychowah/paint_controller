...
def _drain(qt_app, handler: UIHeartbeatHandler) -> None:
    """Stop timer and process queued telemetry applies."""
    handler.cleanup()
...
def test_clear_error_state_publishes_and_resets_runtime_state(qt_app):
    ...
    # Sole outbound from handler: /clear/error (Empty). Heartbeat outbound is PaintRosNode.
    assert node.publishers[0].published_messages[-1].__class__.__name__ == "Empty"
...
def test_heartbeat_callback_does_not_mutate_until_process_events(qt_app):
    node = FakeNode()
    handler = UIHeartbeatHandler(node, state_store=FakeStateStore())
    handler.cleanup()
